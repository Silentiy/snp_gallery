...
async function requestAccessKey(){
    try {
        ...
        return await json_data.access_key;
    } catch {error => 
...
async function makeRequestToUnsplash(){
    ...
    try {
        ...
        const json_data = await response.json();
        json_data.results.forEach( (imageObj) => {
            createCard(imageObj)
        });
    } catch {error => 
...
function createCardVoteButtons(imageObj){
    ...
    let secondButton = votesButtonsDiv.lastChild;
...
function createCardBottomPart(imageObj) {
    ...
    let addedDate = imageObj.created_at.split("T")[0];
    ...
    commentsLink.href = `comments.html?${imageObj.id}`;
